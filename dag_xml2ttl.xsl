...
<xsl:import href="dag_pos.xsl"/>
<xsl:import href="dag_definition_mainsense.xsl"/>
<xsl:import href="dag_definition_multiword.xsl"/>
<xsl:import href="dag_definition_subsense.xsl"/>
<xsl:import href="dag_definition_subsense_idem.xsl"/>
<xsl:import href="dag_terminology.xsl"/>
<xsl:import href="dag_usage.xsl"/>
<xsl:import href="dag_collocation.xsl"/>
<xsl:import href="dag_locution.xsl"/>
<xsl:import href="dag_idem.xsl"/>
<xsl:import href="dag_etymology.xsl"/>
<xsl:import href="dag_autreslangues.xsl"/>
<xsl:import href="dag_proverb.xsl"/>
<xsl:import href="dag_compound.xsl"/>
<xsl:import href="dag_scriptae.xsl"/>
<xsl:import href="dag_hw.xsl"/>
...
<xsl:variable name="mainlemma_URI"><xsl:text disable-output-escaping="yes"><![CDATA[<https://dag.linguistik.uzh.ch/lemme/]]></xsl:text><xsl:value-of select="$mainlemma_name"/>#</xsl:variable>
...
dag:DAG a ontolex:Lexicon , lime:Lexicon;
  ...
  [
	...
	foaf:homepage <xsl:text disable-output-escaping="yes"><![CDATA[<https://hadw-bw.de/forschung/forschungsstelle/dictionnaire-onomasiologique-de-lancien-gascon-dag>]]></xsl:text>
	] ;
  dct:publisher
  [
...
<xsl:choose>
<!-- DEAFplus: unstructured article / emendation article -->
...
dag:DAG_lexresource a lexicog:LexicographicResource ;
  ...
  lexicog:entry dag:<xsl:value-of select="//part[@type='mainpart']/title/lemma"/>_entry .

dag:<xsl:value-of select="//part[@type='mainpart']/title/lemma"/>_entry a lexicog:Entry ;
  rdfs:member dag:<xsl:value-of select="lemma"/>_entity .

dag:<xsl:value-of select="lemma"/>_entity a lexicog:LexicographicComponent ;
  lexicog:describes :<xsl:value-of select="lemma"/> .

dag:DAG lime:entry :<xsl:value-of select="lemma"/> .
</xsl:when>
<xsl:otherwise>
dag:<xsl:value-of select="//part[@type='mainpart']/title/lemma"/>_entry rdfs:member dag:<xsl:value-of select="lemma"/>_entity .
dag:<xsl:value-of select="lemma"/>_entity a lexicog:LexicographicComponent ;
  lexicog:describes <xsl:value-of select="$mainlemma_URI"/><xsl:value-of select="lemma"/>> .
dag:DAG lime:entry <xsl:value-of select="$mainlemma_URI"/><xsl:value-of select="lemma"/>> .
</xsl:otherwise>
</xsl:choose>
</xsl:when>
<!-- DAGpre: normal article (= only type possible), currently not in use -->
...
:<xsl:value-of select="../../../child::title/lemma"/> lemonety:etymology dag:<xsl:value-of select="../../../child::title/lemma"/>_etymology .
dag:<xsl:value-of select="../../../child::title/lemma"/>_etymology a lemonety:Etymology ;
  lemonety:etymology dag:<xsl:for-each select="tokenize(.,' ')">
...
dag:<xsl:for-each select="tokenize(.,' ')">
...
<xsl:if test="not(position() eq last())">_</xsl:if></xsl:for-each>_etymon a lemonety:Etymon ;
  lemonety:isEtymologyOf dag:<xsl:value-of select="../../../child::title/lemma"/>_etymology ;
  dct:language <xsl:call-template name="etymology_extern"/> .
</xsl:for-each></xsl:when>
<xsl:otherwise><xsl:for-each select="m:cited-word[@index='e'][1]">
<xsl:value-of select="$mainlemma_URI"/><xsl:value-of select="../../../child::title/lemma"/>> lemonety:etymology dag:<xsl:value-of select="../../../child::title/lemma"/>_etymology .
dag:<xsl:value-of select="../../../child::title/lemma"/>_etymology a lemonety:Etymology ;
  lemonety:etymology dag:<xsl:for-each select="tokenize(.,' ')">
  <xsl:sequence select="lower-case(replace(replace(replace(., '^\*', 'asterisk_'), '\(|\)|\+|-|,', ''), '/', '_'))"/>
  <xsl:if test="not(position() eq last())">_</xsl:if></xsl:for-each>_etymon .
...
dag:<xsl:value-of select="../../../child::title/lemma"/>_etymology lemonety:etymology dag:<xsl:for-each select="tokenize(.,' ')">
<xsl:sequence select="lower-case(replace(replace(replace(., '^\*', 'asterisk_'), '\(|\)|\+|-|,', ''), '/', '_'))"/>
<xsl:if test="not(position() eq last())">_</xsl:if></xsl:for-each>_etymon .
dag:<xsl:for-each select="tokenize(.,' ')">
<xsl:sequence select="lower-case(replace(replace(replace(., '^\*', 'asterisk_'), '\(|\)|\+|-|,', ''), '/', '_'))"/>
...
<xsl:if test="m:cited-word[@index='a'] | m:remark/m:cited-word[@index='a'] | m:remark/m:evidence/m:quotation/m:comment/m:cited-word[@index='a'] | m:a/m:cited-word[@index='a'] | m:basetym/m:cited-word[@index='a']">
# --- autres langues -----------

<xsl:for-each select="m:cited-word[@index='a'] | m:remark/m:cited-word[@index='a'] | m:remark/m:evidence/m:quotation/m:comment/m:cited-word[@index='a'] | m:a/m:cited-word[@index='a'] | m:basetym/m:cited-word[@index='a']">
...
# graphical variant
:<xsl:value-of select="../../../../child::title/lemma"/> ontolex:otherForm :<xsl:value-of select="../../../../child::title/lemma"/>_form_<xsl:value-of select="replace(., '\(|\)', '')"/> .
:<xsl:value-of select="../../../../child::title/lemma"/>_form_<xsl:value-of select="replace(., '\(|\)', '')"/> a ontolex:Form ;
...
<xsl:if test=".[not(@index='0')]">
# graphical variant
<xsl:value-of select="$mainlemma_URI"/><xsl:value-of select="../../../../child::title/lemma"/>> ontolex:otherForm :<xsl:value-of select="../../../../child::title/lemma"/>_form_<xsl:value-of select="replace(., '\(|\)', '')"/> .
:<xsl:value-of select="../../../../child::title/lemma"/>_form_<xsl:value-of select="replace(., '\(|\)', '')"/> a ontolex:Form ;
	ontolex:writtenRep "<xsl:value-of select="."/>"<xsl:call-template name="scriptae_extern"/> .
