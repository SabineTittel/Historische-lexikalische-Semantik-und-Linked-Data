...
<xsl:template name="collocation_extern">
# collocation "<xsl:value-of select="replace(., '\+', '')"/>"
...
<xsl:otherwise>"collocation: error in the main-lemma-sub-lemma structure"@en</xsl:otherwise>
</xsl:choose></xsl:when>
<!-- sub sense -->
<xsl:when test="name(../../../parent::node()) = 'senses'">
<xsl:choose><xsl:when test="../../../../..[@type='mainpart']">:<xsl:value-of select="//part[@type='mainpart']/title/lemma"/></xsl:when>
...
<xsl:otherwise>"collocation: error in the main-sense-sub-sense-structure"@en</xsl:otherwise>
</xsl:choose> ;
